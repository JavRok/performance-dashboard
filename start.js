...
// Load Storage system and pass it to other scripts
const storage = conf.getStorage().then(storage => {
	...
	const checkInterval = setInterval (() => {
		CheckForTests.run(storage);
	}, checkTimeout);


	// Save history every 24h
	const historyInterval = setInterval (() => {
		SaveHistory.run(storage);
	}, saveHistoryTimeout);
