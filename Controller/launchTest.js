...
async function run(storage) {
	try {
		...
		// await locations.initFromStorage();
		const existingTests = await storage.getPendingTests();
		...
		sites.forEach ( url => {
			...
			if (existingTests.has(url)) {
				...
				if (Array.isArray(existing)) {
					...
					testStatus.getStatusMultiple(existing)
						.then(statuses => {
							...
							let bestExisting = statuses.reduce(function (acc, status) {
								...
								return (status.position < acc.position) ? status : acc;
							});
					...
					testStatus.getStatus(existing).then(status => {
						// If the queue is too long, launch the test in a different server
